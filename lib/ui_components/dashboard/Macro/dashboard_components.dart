...
import 'package:auth/ui_components/dashboard/Macro/splitForm.dart';
      ...
      if (snapshot.connectionState == ConnectionState.waiting) {
        ...
        return Row(
          children: [
            ...
            const Spacer(),
            Text(
              'Daily Proteins :\n${data['DailyCal'] * (data['Protein split']/100) ~/ 4} g', // 27% of calories as proteins
              style: GoogleFonts.oswald(color: Colors.black, fontSize: 16),
              textAlign: TextAlign.center,
            ),
            const Spacer(),
            Text(
              'Daily Carbs:\n${data['DailyCal'] * (data['Carbs split']/100) ~/ 4} g', // 47% of calories as carbs
              style: GoogleFonts.oswald(color: Colors.black, fontSize: 16),
              textAlign: TextAlign.center,
            )
          ],
...
class SplitLegend extends StatefulWidget {
  const SplitLegend({
    Key? key,
    required this.index,
    required this.name,
  ...
  @override
  State<SplitLegend> createState() => _SplitLegendState();
}

class _SplitLegendState extends State<SplitLegend> {
  @override
  Widget build(BuildContext context) {
    return Row(
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            color: kColors.elementAt(widget.index % kColors.length),
          ),
        ),
